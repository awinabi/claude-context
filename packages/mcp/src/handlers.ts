...
export class ToolHandlers {
    ...
    public async handleSearchCode(args: any) {
        ...
        try {
            ...
            console.log(`[SEARCH] ✅ Search completed! Found ${searchResults.length} results`);
    ...
    public async handleReindexSubpath(args: any) {
        const { path: codebasePath, subpath } = args;
        ...
        try {
            ...
            // Validate codebase is already indexed
            const isIndexed = this.snapshotManager.getIndexedCodebases().includes(absolutePath);
            if (!isIndexed) {
                return {
                    content: [{
            ...
            if (!subpath || subpath.trim() === '') {
                return {
                    content: [{
                        type: "text",
                        text: `Error: subpath parameter is required.`
                    }],
                    isError: true
                };
            }

            const fullSubpath = path.join(absolutePath, subpath);
            if (!fs.existsSync(fullSubpath)) {
                return {
                    content: [{
                        type: "text",
                        text: `Error: Sub-path '${fullSubpath}' does not exist.`
                    }],
                    isError: true
                };
            }
            ...
            if (!stat.isDirectory()) {
                return {
                    content: [{
                        type: "text",
                        text: `Error: Sub-path '${fullSubpath}' is not a directory.`
                    }],
                    isError: true
                };
            }
            ...
            // Start background sub-path re-indexing
            this.startBackgroundSubpathReindex(absolutePath, subpath);

            return {
                content: [{
                    type: "text",
                    text: `Started background re-indexing for sub-path '${subpath}' in codebase '${absolutePath}'.\n\nRe-indexing is running in the background. You can search the codebase while re-indexing is in progress.`
                }]
            };
            ...
            return {
                content: [{
                    type: "text",
                    text: `Error starting sub-path re-indexing: ${error.message || error}`
                }],
                isError: true
            };
        }
    ...
    private async startBackgroundSubpathReindex(codebasePath: string, subpath: string) {
        let lastSaveTime = 0;

        try {
            console.log(`[BACKGROUND-REINDEX] Starting sub-path re-indexing: ${subpath} in ${codebasePath}`);

            await this.context.getLoadedIgnorePatterns(codebasePath);

            const stats = await this.context.reindexSubpath(codebasePath, subpath, (progress) => {
                ...
                console.log(`[BACKGROUND-REINDEX] Progress: ${progress.phase} - ${progress.percentage}%`);
            });

            console.log(`[BACKGROUND-REINDEX] ✅ Sub-path re-indexing completed! Files: ${stats.indexedFiles}, New chunks: ${stats.totalChunks}, Deleted chunks: ${stats.deletedChunks}`);

            // Restore indexed status with updated stats
            const existingInfo = this.snapshotManager.getCodebaseInfo(codebasePath) as any;
            const updatedStats = {
                indexedFiles: existingInfo?.indexedFiles || stats.indexedFiles,
                totalChunks: existingInfo?.totalChunks || stats.totalChunks,
                status: 'completed' as const
            };
            this.snapshotManager.setCodebaseIndexed(codebasePath, updatedStats);
            this.snapshotManager.saveCodebaseSnapshot();

        } catch (error: any) {
            console.error(`[BACKGROUND-REINDEX] Error during sub-path re-indexing:`, error);

            const lastProgress = this.snapshotManager.getIndexingProgress(codebasePath);
            this.snapshotManager.setCodebaseIndexFailed(codebasePath, error.message || String(error), lastProgress);
            this.snapshotManager.saveCodebaseSnapshot();
        }
    }
}
